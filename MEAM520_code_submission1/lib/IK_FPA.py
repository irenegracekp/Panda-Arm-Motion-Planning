...
class IK:
    ...
    def ik_pos(self, wrist_pos):
        ...
        #Joint angle 7
        # q71 = ((np.arctan2(-wrist_pos[1,0],wrist_pos[0,0])) + pi/4) #First possible value of q71
        # q72 = q71 + pi
        ...
        o27_n1 = (np.array([np.cos(q71-pi/4), -np.sin(q71-pi/4), 0, np.sin(q71-pi/4), np.cos(q71-pi/4), 0, 0, 0, 1]).reshape(3,3)) @ wrist_pos
        o27_n2 = (np.array([np.cos(q72-pi/4), -np.sin(q72-pi/4), 0, np.sin(q72-pi/4), np.cos(q72-pi/4), 0, 0, 0, 1]).reshape(3,3)) @ wrist_pos

        # print(o27_n1.shape)

        j7j2_1 = [o27_n1[0,0], o27_n1[2,0]]
        j7j2_2 = [o27_n2[0,0], o27_n2[2,0]]

        j6j2_1 = j7j2_1 + [self.a6, self.d7]
        j6j2_2 = j7j2_2 + [self.a6, self.d7]

        j6j21 = np.sqrt((j6j2_1[0]**2 + j6j2_1[1]**2))
        j6j22 = np.sqrt((j6j2_2[0]**2 + j6j2_2[1]**2))

        l1 = np.sqrt((self.d5**2 + self.a4**2))
        l2 = np.sqrt((self.d3**2 + self.a3**2))

        alpha = np.arccos((l2**2 + l1**2 - (self.d3 + self.d5)**2)/(2*l1*l2))

        beta = np.arctan2(self.a4, self.d5)

        q41 = alpha - np.arccos((l1**2 + l2**2 - j6j21**2)/ (2*l1*l2))
        q42 = alpha + np.arccos((l1**2 + l2**2 - j6j21**2)/ (2*l1*l2))
        q43 = alpha - np.arccos((l1**2 + l2**2 - j6j22**2)/ (2*l1*l2))
        q44 = alpha + np.arccos((l1**2 + l2**2 - j6j22**2)/ (2*l1*l2))

        q61 = beta + np.arctan2(-o27_n1[0,0] - self.a6, o27_n1[2,0] + self.d7) - np.arccos((l1**2 + j6j21**2 - l2**2)/(2*j6j21*l1))
        q62 = beta + np.arctan2(-o27_n1[0,0] - self.a6, o27_n1[2,0] + self.d7) + np.arccos((l1**2 + j6j21**2 - l2**2)/(2*j6j21*l1))
        q63 = beta + np.arctan2(-o27_n2[0,0] - self.a6, o27_n2[2,0] + self.d7) - np.arccos((l1**2 + j6j22**2 - l2**2)/(2*j6j22*l1))
        q64 = beta + np.arctan2(-o27_n2[0,0] - self.a6, o27_n2[2,0] + self.d7) + np.arccos((l1**2 + j6j22**2 - l2**2)/(2*j6j22*l1))
